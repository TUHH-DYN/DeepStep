...
from modules.process_waveeqn_fem_data import process_waveeqn_fem_data 
...
def process_waveeqn_data(abs_path, config):

    print('cfl_fem: '+str(config['cfl_fem']))
    print('cfl: '+str(config['cfl']))
    ...
    [mesh, values] = process_waveeqn_fem_data(output_path, config['filename_base'], config['time_end'], config['num_steps_fem'], config['skip_fem_steps_for_output'], config['num_fem_cells'], domain_list, no_domain_bc_expression, no_domain_material_expression)
